...
void SlamManager::run()
{
    ...
    while( !bexit_required_ ) {
        ...
        if( getNewImage(img_left, img_right, time) )
        {
            ...
            if( frame_id_ > 0 ) {
                cam_delay = std::chrono::duration<double>(std::chrono::steady_clock::now().time_since_epoch()).count() - last_img_time;
                last_img_time += cam_delay;
            } else {
                last_img_time = std::chrono::duration<double>(std::chrono::steady_clock::now().time_since_epoch()).count();
            }
            ...
            // ========================================
            bool c1 = cam_delay > 0;
            bool c2 = ( std::chrono::duration<double>(std::chrono::steady_clock::now().time_since_epoch()).count() - last_img_time ) > 100. * cam_delay;
